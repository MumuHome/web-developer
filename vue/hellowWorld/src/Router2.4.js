  ...
  routes:[
    ...
    {path:'/first',name:'home-first',component:child,
      children:[
        ...
        // 把路径third的路径重定义为first
        {path:'third',redirect:'first'}
      ]
    },
    // 在后面加上alias:'/go' 把/go的地址 指向/second
    {path:'/second',name:'home-second',component:second,alias:'/go'},
    {path:'/aaa/:id',component:firstFirst},
    // 把路径/bbb/:id的路径重定义为/aaa/:id
    {path:'/bbb/:id',redirect:'/aaa/:id'},
    {
      path:'/ccc/:id',
      // 箭头函数定义 如果这个地址中的id=001，那么把这个地址重定向到'/'中
      redirect:xxx => {
  ...
  template:`
    <div id="r">
      ...
      <ol>
          ...
          <li><router-link to="/aaa/xuj">aaa</router-link></li>
          <li><router-link to="/bbb/wuq">bbb</router-link></li>
          <li><router-link to="/ccc/001">001</router-link></li>
      </ol>
      <!--显示在这里-->
      <router-view class="main"></router-view>
    </div>
